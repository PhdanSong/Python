...
def conv2d(x,W):
    ...
    #strides[0]=strides[3]=1 strides[1]代表x方向步长，strides[2]表示y方向步长
    #padding:A 'string' from "SAME" "VALID"
    return tf.nn.conv2d(x,W,strides=[1,1,1,1],padding='SAME')

#池化层
def max_pool_2x2(x):
    #ksize[1,x,y,1],ksize窗口大小，strides是步长，x的格式同上
...
#把x_image和权值向量进行卷积，再加上权值，然后应用于relu激活函数
h_conv1 = tf.nn.relu(conv2d(x_image,W_conv1)+b_conv1)
h_pool1 = mac=max_pool_2x2(h_conv1) #进行max_pooling
...
#结果放在一个布尔列表中
correct_prediction = tf.equal(tf.argmax(prediction,1),tf.argmax(y,1))#argmax返回一维向量中最大的值所在的位置
#求准确率
